...
def ModelStart(userData):
    measurements_per_rotation, number_of_beams = 1200, 128
    OutputFormat = 'time@i,%d@[,x@f,y@f,z@f,intensity@f' % (measurements_per_rotation * number_of_beams)
    userData['sensor'] = BusAccessor(userData['busId'], 'SolidStateLidarPointCloudSensor.0', OutputFormat)
    userData['last_time'] = 0
    plt.ion()
    plt.figure(dpi=100).canvas.set_window_title('PanoSim HowTo Sensor: Solid State Lidar Point Cloud')
...
def ModelOutput(userData):
    ...
    if timestamp > userData['last_time']:
        ...
        plt.cla()
        plt.title('Sensor: Solid State Lidar Point Cloud')
